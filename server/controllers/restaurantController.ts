...
export const getRestaurants = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const restaurant = await Restaurant222.find(queryObj).sort(sortOption)
        res.json(restaurant)
    } catch (error: any) {
        console.error(error)
...
export const getFeaturedRestaurants = async (req: Request, res: Response): Promise<void> => {
    try {
        const featured = await Restaurant222.find({
            status: 'approved',
...
export const getRestaurantBySlug = async (req: Request, res: Response): Promise<void> => {
    try {
        const restaurant = await Restaurant222.findOne({ slug: req.params.slug })
        if (!restaurant) {
